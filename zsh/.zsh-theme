...
# React
if [ ! -n "${BULLETTRAIN_REACT_BG+1}" ]; then
  BULLETTRAIN_REACT_BG="#61DBFB"
fi
if [ ! -n "${BULLETTRAIN_REACT_FG+1}" ]; then
  BULLETTRAIN_REACT_FG=black
fi
if [ ! -n "${BULLETTRAIN_REACT_PREFIX+1}" ]; then
  BULLETTRAIN_REACT_PREFIX=""
fi

# Preact
if [ ! -n "${BULLETTRAIN_PREACT_BG+1}" ]; then
  BULLETTRAIN_PREACT_BG="#673ab7"
fi
if [ ! -n "${BULLETTRAIN_PREACT_FG+1}" ]; then
  BULLETTRAIN_PREACT_FG=white
fi
if [ ! -n "${BULLETTRAIN_PREACT_PREFIX+1}" ]; then
  BULLETTRAIN_PREACT_PREFIX=""
fi

# Astro
if [ ! -n "${BULLETTRAIN_ASTRO_BG+1}" ]; then
  BULLETTRAIN_ASTRO_BG="#FF5D01"
fi
if [ ! -n "${BULLETTRAIN_ASTRO_FG+1}" ]; then
  BULLETTRAIN_ASTRO_FG=white
fi
if [ ! -n "${BULLETTRAIN_ASTRO_PREFIX+1}" ]; then
  BULLETTRAIN_ASTRO_PREFIX=""
fi
...
prompt_react() {
  if [[ -f package.json ]] && grep -q '"react"' package.json 2>/dev/null; then
    if command -v node > /dev/null 2>&1; then
      local react_version=$(grep -o '"react": "[^"]*"' package.json 2>/dev/null | cut -d'"' -f4)
      prompt_segment $BULLETTRAIN_REACT_BG $BULLETTRAIN_REACT_FG $BULLETTRAIN_REACT_PREFIX" $react_version"
    fi
  fi
}
...
# Preact
prompt_preact() {
  if [[ -f package.json ]] && grep -q '"preact"' package.json 2>/dev/null; then
    if command -v node > /dev/null 2>&1; then
      local preact_version=$(grep -o '"preact": "[^"]*"' package.json 2>/dev/null | cut -d'"' -f4)
      prompt_segment $BULLETTRAIN_PREACT_BG $BULLETTRAIN_PREACT_FG $BULLETTRAIN_PREACT_PREFIX" $preact_version"
    fi
  fi
}
...
prompt_astro() {
  if [[ -f package.json ]] && grep -q '"astro"' package.json 2>/dev/null; then
    if command -v node > /dev/null 2>&1; then
      local astro_version=$(grep -o '"astro": "[^"]*"' package.json 2>/dev/null | cut -d'"' -f4)
      prompt_segment $BULLETTRAIN_ASTRO_BG $BULLETTRAIN_ASTRO_FG $BULLETTRAIN_ASTRO_PREFIX" $astro_version"
    fi
  fi
}
